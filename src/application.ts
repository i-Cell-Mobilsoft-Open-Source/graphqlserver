...
export class GqlApplication extends BootMixin(
  ...
  parseArgs(): GqlApplication {
    ...
    const kvTrippleTest = (v: any) => /^(header|query|path|cookie):[a-z0-9_\-\/]+:[a-z0-9_\-\/]+$/i.test(v);
    ...
    this.argv = yargs
      ...
      .option('header', {
        type: 'string',
        describe: 'A custom HTTP header to be passed to each request generated by GQL. Format: headerName:value. You may supply this flag multiple times.'
      })
      .option('queryStr', {
        type: 'string',
        describe: 'A custom HTTP query string parameter to be passed to each request generated by GQL. Format: name:value. You may supply this flag multiple times'
      })
      .option('query', {
        type: 'string',
      ...
      .option('param', {
        type: 'string',
        describe: 'A custom parameter to be added to each OAS operation. Format: type:schema:name. A required param will be added to the OAS parameters object. Values must specified by the user, at runtime. You may supply this flag multiple times.'
      })
      .implies('key', 'cert')
      .implies('cert', 'key')
      .normalize('key')
      ...
      .check((argv: any) => {
        setTimeout(() => {
          if (!argv.oas.every(urlOrFileTest)) {
            ...
            throw new Error(`One or more OAS params are not fromatted correctly: "${argv.param.join(',')}"`);
          }
        })
        return true;
      }, true)
      ...
      .example('npm start -- --baseUrl http://127.0.0.1:3000/api', 'Node server mode')
      .example('npm start -- --oas http://some.domain.com:10001/openapi  --header Content-Type:application/json', 'GraphQL layer covering a single REST API')
      .example('npm start -- --oas http://some.domain.com:10001/openapi -o http://some.domain.com:10008/openapi -baseUrl http://some.domain.com --header sessionToken:someToken --header login:someUser --header Content-Type:application/json', 'GraphQL layer covering multiple REST APIs and auth')
      .showHelpOnFail(true)
