...
import { parse, printSchema, visit, buildASTSchema, GraphQLSchema } from 'graphql';
...
export class GraphQLService {
  ...
  createOperationMapping(openAPISchemas: (Oas3 | Oas2)[], opsMap: AnyObject[] = []): AnyObject {
    ...
    for (const schema of openAPISchemas) {
      ...
      for (const path of pathKeys) {
        const op: any = opsMap.find((op) => {
          return op.path === path && Object.keys(schema.paths[path]).includes(op.method);
        });
  ...
  async createSchema(cliArgs: AnyObject, openAPISchemas: (Oas3 | Oas2)[], opsMap: AnyObject[] = []): Promise<any> {
    const gqlOptions: AnyObject = {
      ...
      selectQueryOrMutationField: this.createOperationMapping(openAPISchemas, opsMap)
    }

    const { schema, report } = await createGraphQlSchema(openAPISchemas, gqlOptions);
